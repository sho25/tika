...
argument_list|)
decl_stmt|;
comment|//this can't be static because of the ForkParser
comment|//lazily load this when parse is called if it is null.
specifier|private
name|MediaTypeRegistry
name|bufferedMediaTypeRegistry
decl_stmt|;
specifier|private
specifier|final
name|Object
...
name|TikaException
block|{
comment|//lazily load the MediaTypeRegistry at parse time
comment|//only want to call getDefaultConfig() once, and can't
comment|//load statically because of the ForkParser
name|TikaConfig
name|config
init|=
name|context
...
condition|)
block|{
comment|//buffer this for next time.
synchronized|synchronized
init|(
name|lock
init|)
block|{
comment|//now that we're locked, check again
if|if
condition|(
name|bufferedMediaTypeRegistry
operator|==
...
return|return;
block|}
comment|//if the existing type is a specialization of the detected type,
comment|//leave in the specialization; otherwise set the detected
if|if
condition|(
operator|!
name|mediaTypeRegistry
