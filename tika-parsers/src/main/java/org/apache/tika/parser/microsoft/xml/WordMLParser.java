...
condition|)
block|{
comment|//src is an internal designator with an extension
name|String
name|src
init|=
name|attrs
...
argument_list|)
decl_stmt|;
comment|//title appears to be the original file name
name|String
name|title
init|=
name|attrs
...
condition|)
block|{
comment|//take the extention from the src and append it to the title
name|int
name|i
init|=
name|src
...
return|return;
block|}
comment|//somewhat tricky...
comment|//can't just dump bin_data at the end of the
comment|//bin_data element because there may be metadata
comment|//after it, if it is within a pict element
comment|//<pict><binData></binData><imagedata/></pict>.
comment|//However, if you aren't in a pict (say docOLEdata), then do dump binary
comment|//data at the end of the bin data.
if|if
condition|(
name|PICT
operator|.
