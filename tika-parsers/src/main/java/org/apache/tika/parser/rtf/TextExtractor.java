...
name|minute
decl_stmt|;
comment|//This keeps track of the following elements as they are
comment|//written to the handler: p, li, ol, ul
comment|//This tries to prevent malformed tag orders in the RTF
comment|//e.g.<p></li></ol></p>
comment|//from generating malformed xml tags. (TIKA-2899)
comment|//This may conceal problems with our parser.
comment|//TODO:
comment|// 1) do we need to add all elements, a, b, i, etc.
comment|// 2) are we doing the right thing by ignoring an element
comment|//    if its match doesn't pop off the stack...or should
comment|//    we pop all at the first failure.
specifier|private
name|Stack
argument_list|<
name|String
...
argument_list|()
decl_stmt|;
comment|//this is an arbitrary limit on the size of the stack
comment|//to defend against DoS with memory consumption
specifier|private
name|int
name|maxStackSize
init|=
...
argument_list|)
expr_stmt|;
comment|//close out whatever tags were left
while|while
condition|(
name|paragraphStack
operator|.
...
block|{
comment|//ignore.  Something is very, very wrong...
block|}
block|}
specifier|private
name|void
...
block|{
comment|//there should have been a starting li
block|}
block|}
else|else
block|{
...
block|}
block|}
comment|//if there was a failure in tag alignment,
comment|//dump all tags and start fresh.
if|if
condition|(
name|badTagAlignment
condition|)
...
block|{
comment|//stack as empty, the list was never started
block|}
block|}
block|}
