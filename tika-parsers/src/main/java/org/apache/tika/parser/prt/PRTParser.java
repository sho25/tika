...
return|;
block|}
comment|/**      * How long do we allow a text run to claim to be, before we      * decide we're confused and it's not really text after all?      */
specifier|private
specifier|static
specifier|final
name|int
...
argument_list|)
expr_stmt|;
comment|// The description, if set, is the next up-to-500 bytes
name|byte
index|[]
name|desc
init|=
...
expr_stmt|;
block|}
comment|/**      * Does our best to turn the bytes into text      */
specifier|private
name|String
name|extractText
parameter_list|(
...
name|TikaException
block|{
comment|// The text is always stored null terminated, but sometimes
comment|//  may have extra null padding too
name|int
name|length
init|=
name|data
...
block|}
block|}
comment|// We believe that the text is basically stored as CP437
comment|// That said, there are a few characters slightly wrong for that...
name|String
name|text
decl_stmt|;
try|try
...
name|TikaException
argument_list|(
literal|"JVM Broken, core codepage CP437 missing!"
argument_list|)
throw|;
block|}
comment|// Fix up the known character issues
name|text
operator|=
name|text
operator|.
...
argument_list|)
expr_stmt|;
comment|// All done, as best as we can!
return|return
name|text
return|;
block|}
