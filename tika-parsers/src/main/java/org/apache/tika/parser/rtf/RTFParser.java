...
return|;
block|}
comment|/**      * Bytes for embedded objects are currently cached in memory.      * If something goes wrong during the parsing of an embedded object,      * it is possible that a read length may be crazily too long      * and cause a heap crash.      *      * @param max maximum number of bytes to allow for embedded objects.  If      *            the embedded object has more than this number of bytes, skip it.      * @deprecated use {@link #setMemoryLimitInKb(int)} instead      */
annotation|@
name|Deprecated
specifier|public
specifier|static
...
expr_stmt|;
block|}
comment|//get rid of this once we get rid of the other static maxbytes...
specifier|private
specifier|static
specifier|volatile
name|boolean
...
parameter_list|()
block|{
comment|//there's a race condition here, but it shouldn't matter.
if|if
condition|(
name|USE_STATIC
condition|)
