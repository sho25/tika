...
expr_stmt|;
block|}
comment|/*     Jenkins failed to build after adding this.     Need to determine if this is the cause, and, if so, how to test for     missing beans.     //TIKA-792; with room for future missing bean tests     public void testWordMissingOOXMLBeans() throws Exception{         //If a bean is missing, POI prints stack trace to stderr          String[] fileNames = new String[]{             "testWORD_missing_ooxml_bean1.docx",//TIKA-792         };         PrintStream origErr = System.err;         for (String fileName : fileNames){             ByteArrayOutputStream errContent = new ByteArrayOutputStream();             System.setErr(new PrintStream(errContent));             Metadata metadata = new Metadata();              ContentHandler handler = new BodyContentHandler();             ParseContext context = new ParseContext();             InputStream input = getTestDocument(fileName);             parser.parse(input, handler, metadata, context);                  String err = errContent.toString();             assertTrue(err.length() == 0);             input.close();         }         System.setErr(origErr);     }*/
block|}
end_class
