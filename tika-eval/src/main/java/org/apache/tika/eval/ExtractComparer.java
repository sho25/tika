...
decl_stmt|;
name|String
name|sharedDigestKey
init|=
name|findSharedDigestKey
argument_list|(
name|metadataListA
argument_list|,
name|metadataListB
...
argument_list|)
decl_stmt|;
comment|//arbitrary decision...only run the comparisons if there are> 10 tokens total
comment|//We may want to bump that value a bit higher?
...
block|}
block|}
comment|/**      * Checks only the first item in each list. Returns the first      * digest key shared by both, if it exists, null otherwise.      * @param metadataListA      * @param metadataListB      * @return      */
specifier|private
name|String
name|findSharedDigestKey
parameter_list|(
...
return|;
block|}
comment|/**      * Try to find the matching metadata based on the AbstractRecursiveParserWrapperHandler.EMBEDDED_RESOURCE_PATH      * If you can't find it, return -1;      *      * @param aIndex                index for match in metadataListA      * @param metadataListA      * @param metadataListB      * @return      */
specifier|private
name|int
name|getMatch
parameter_list|(
...
return|;
block|}
comment|//assume first is always the container file
if|if
condition|(
name|aIndex
operator|==
...
condition|)
block|{
comment|//first try to find matching digests
comment|//this does not elegantly handle multiple matching digests
return|return
name|findMatchingDigests
argument_list|(
name|sharedDigestKey
argument_list|,
name|handledB
...
return|;
block|}
comment|//assume same embedded resource path.  Not always true!
name|Metadata
name|thisMetadata
init|=
name|metadataListA
...
block|}
block|}
comment|//last resort, if lists are same size, guess the same index
if|if
condition|(
name|metadataListA
operator|.
