...
block|}
block|}
comment|//    private static CompositeParser parserFromDomElement(
comment|//            Element element, MimeTypes mimeTypes, ServiceLoader loader)
comment|//            throws TikaException, IOException {
comment|//        List<Parser> parsers = new ArrayList<Parser>();
comment|//
comment|//        // Find the parser children of the parsers tag, if any
comment|//        for (Element pe : getTopLevelElementChildren(element, "parsers", "parser")) {
comment|//            parsers.add(parserFromParserDomElement(pe, mimeTypes, loader));
comment|//        }
comment|//
comment|//        if (parsers.isEmpty()) {
comment|//            // No parsers defined, create a DefaultParser
comment|//            return getDefaultParser(mimeTypes, loader);
comment|//        } else if (parsers.size() == 1&& parsers.get(0) instanceof CompositeParser) {
comment|//            // Single Composite defined, use that
comment|//            return (CompositeParser)parsers.get(0);
comment|//        } else {
comment|//            // Wrap the defined parsers up in a Composite
comment|//            MediaTypeRegistry registry = mimeTypes.getMediaTypeRegistry();
comment|//            return new CompositeParser(registry, parsers);
comment|//        }
comment|//    }
specifier|private
specifier|static
...
throw|;
block|}
comment|// Is this a composite or decorated parser? If so, support recursion
if|if
condition|(
name|CompositeParser
operator|.
...
argument_list|()
decl_stmt|;
comment|// Find the detector children of the detectors tag, if any
for|for
control|(
name|Element
name|de
...
name|add
argument_list|(
name|detectorFromDetectorDomElement
argument_list|(
name|de
argument_list|,
name|mimeTypes
...
condition|)
block|{
comment|// No detectors defined, create a DefaultDetector
return|return
name|getDefaultDetector
argument_list|(
name|mimeTypes
...
block|{
comment|// Wrap the defined detectors up in a Composite
name|MediaTypeRegistry
name|registry
init|=
name|mimeTypes
...
name|IOException
block|{
comment|// TODO Do this properly
comment|// TODO This is a cheat for parsers only!
return|return
operator|(
name|T
operator|)
