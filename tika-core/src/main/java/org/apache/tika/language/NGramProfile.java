...
literal|null
decl_stmt|;
comment|/** The NGrams of this profile sorted on the number of occurences */
specifier|private
name|List
argument_list|<
name|NGramEntry
...
block|}
block|}
comment|/**      * Normalize the profile (calculates the ngram frequencies)      */
specifier|protected
name|void
name|normalize
parameter_list|()
block|{
comment|// Calculate ngram count if not already done
if|if
condition|(
name|ngramcounts
operator|==
...
parameter_list|)
block|{
comment|// e.printStackTrace(LogUtil.getWarnStream(LOG));
block|}
name|newProfile
operator|.
name|analyze
...
argument_list|()
operator|+
literal|" for Nutch Language Identification\n"
operator|)
operator|.
name|getBytes
argument_list|()
