...
condition|)
block|{
comment|// Empty document, so treat it as binary
...
condition|)
block|{
comment|// No control characters, so treat it as text
return|return
name|MediaType
operator|.
name|TEXT_PLAIN
...
condition|)
block|{
comment|// Almost plain text (< 2% control,> 90% ASCII range)
comment|// See https://issues.apache.org/jira/browse/TIKA-688
return|return
name|MediaType
operator|.
name|TEXT_PLAIN
